...
public class ChatTierConfigValidator implements InitializingBean {
    ...
    /**
     * 校验档位引用是否有效（default-tier / deep-thinking-tier 必须指向已配置的档位）
     *
     * @param tierName 档位名称（配置值）
     * @param label    配置项标签（用于错误消息）
     * @param tiers    所有已配置的档位
     * @param errors   错误收集列表
     */
    ...
    /**
     * 逐档位校验：timeout-ms 必填且为正数；候选列表非空且所有引用 id 必须在注册表中可查
     *
     * @param tiers       所有已配置的档位
     * @param registryIds 候选注册表中所有已知 id（用于交叉校验引用完整性）
     * @param errors      错误收集列表
     */
