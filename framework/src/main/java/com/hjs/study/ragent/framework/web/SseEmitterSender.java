 ...
 *
 * <p>该类对 Spring 的 SseEmitter 进行封装，统一处理连接关闭状态和异常情况。
 * 它重点保证“终态只执行一次”：流式回答可能同时遇到完成、超时、客户端断开和模型异常，
 * 没有终态保护就会出现重复 complete 或在已关闭连接上继续发送的错误。</p>
 */
@Slf4j
public class SseEmitterSender {
    ...
    public SseEmitterSender(SseEmitter emitter) {
        ...
        // 三种容器回调都会把本地状态同步为关闭，之后的业务回调只需快速返回。
    ...
    public void complete() {
        // CAS 让 onComplete、超时回调和取消回调竞争时只有一个能真正关闭连接。
    ...
    private void closeWithError(Throwable throwable) {
        // 与正常完成共享同一个 closed 标志，异常和成功不能各自完成一次。
        if (closed.compareAndSet(false, true)) {
