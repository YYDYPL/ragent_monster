 ...
 * 全局异常到统一响应的映射器。
 * <p>
 * 它把参数校验、业务异常、认证异常和未知异常收敛为 {@link Result}，既保证前端协议稳定，
 * 又在服务端保留足够日志用于排障。已经开始写入的 SSE 响应不能再安全地改成 JSON 响应，
 * 因而流式链路在 {@link SseEmitterSender} 内部完成异常收尾。
 * </p>
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    public Result<Void> validExceptionHandler(HttpServletRequest request, MethodArgumentNotValidException ex) {
        ...
        // 参数不合法属于调用方可修复的问题，返回客户端错误而不是服务端错误。
    ...
    public Result<Void> abstractException(HttpServletRequest request, AbstractException ex) {
        if (ex.getCause() != null) {
            // 有底层原因时记录完整异常链，返回体仍只暴露项目定义的错误码与安全消息。
    ...
    public Result<Void> defaultErrorHandler(HttpServletRequest request, Throwable throwable) {
        ...
        // 未分类异常不能把堆栈直接返回浏览器，避免泄露内部实现和基础设施信息。
